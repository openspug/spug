  ...
  render() {
    ...
    return (
      <Modal
        ...
        maskClosable={false}>
        <SearchForm>
          <SearchForm.Item span={8} title={t('模板类别')}>
            <Select allowClear placeholder={t('请选择')} value={store.f_type} onChange={v => store.f_type = v}>
              {store.types.map(item => (
                <Select.Option value={item} key={item}>{item}</Select.Option>
              ))}
            </Select>
          </SearchForm.Item>
          <SearchForm.Item span={8} title={t('模板名称')}>
            <Input allowClear value={store.f_name} onChange={e => store.f_name = e.target.value} placeholder={t('请输入')}/>
          </SearchForm.Item>
          <SearchForm.Item span={8}>
            <Button type="primary" icon={<SyncOutlined/>} onClick={store.fetchRecords}>{t('刷新')}</Button>
          </SearchForm.Item>
        </SearchForm>
        <Table
          rowKey="id"
