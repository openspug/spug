...
function NavForm(props) {
  ...
  function handleSubmit() {
    ...
    if (links.length === 0) return message.error(t('请设置至少一条导航链接'));
    if (fileList.length === 0) return message.error(t('请上传导航logo'));
  ...
  function beforeUpload(file) {
    if (file.size / 1024 > 100) {
      message.error(t('图片将直接存储至数据库，请上传小于100KB的图片'));
  ...
  return (
    <Modal
      ...
      <Form form={form} initialValues={record} labelCol={{span: 5}} wrapperCol={{span: 18}}>
        <Form.Item required label={t('导航图标')}>
          <Upload
            accept="image/*"
            ...
            {fileList.length === 0 && (
              <div>
                <PlusOutlined/>
                <div style={{marginTop: 8}}>{t('点击上传')}</div>
              </div>
            )}
          </Upload>
        ...
        <Form.Item required name="title" label={t('导航标题')}>
          <Input placeholder={t('请输入')}/>
        </Form.Item>
        <Form.Item required name="desc" label={t('导航描述')}>
          <Input placeholder={t('请输入')}/>
        </Form.Item>
        <Form.Item required label={t('导航链接')} style={{marginBottom: 0}}>
          {record.links.map((item, index) => (
            <div key={index} style={{display: 'flex', alignItems: 'center', marginBottom: 12}}>
              <Form.Item style={{display: 'inline-block', margin: 0, width: 100}}>
                <Input value={item.name} onChange={e => changeLink(e, index, 'name')} placeholder={t('链接名称')}/>
              </Form.Item>
              <Form.Item style={{display: 'inline-block', width: 210, margin: '0 8px'}}>
                <Input value={item.url} onChange={e => changeLink(e, index, 'url')} placeholder={t('请输入链接地址')}/>
              </Form.Item>
