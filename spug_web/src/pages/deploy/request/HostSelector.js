...
export default observer(function (props) {
  ...
  function DeployStatus(props) {
    switch (props.status) {
      ...
      case '1':
        return <Tag color="orange">{t('发布中')}</Tag>
      case '2':
        return <Tag color="green">{t('发布成功')}</Tag>
      case '3':
        return <Tag color="red">{t('发布失败')}</Tag>
      default:
        return <Tag color="blue">{t('待发布')}</Tag>
    }
  }
  ...
  return (
    <Modal
      ...
      <Table
        ...
        <Table.Column title={t('主机名称')} dataIndex="name"/>
        <Table.Column title={t('连接地址')} dataIndex="hostname"/>
        <Table.Column title={t('备注信息')} dataIndex="desc"/>
        {props.deploy_status ? (
          <Table.Column title={t('发布状态')} render={v => <DeployStatus status={props.deploy_status[v.id]}/>}/>
        ) : null}
      </Table>
    </Modal>
  )
