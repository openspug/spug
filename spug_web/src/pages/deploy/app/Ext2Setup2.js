...
class Ext2Setup2 extends React.Component {
  constructor(props) {
    ...
    this.helpMap = {
      '0': null,
      '1': t('相对于输入的本地路径的文件路径，仅将匹配到文件传输至要发布的目标主机。'),
      '2': t('支持模糊匹配，基于输入的本地路径匹配，匹配到文件将不会被传输。')
    }
    this.state = {
      loading: false,
    }
      ...
      <Form labelCol={{span: 6}} wrapperCol={{span: 14}} className={styles.ext2Form}>
        {store.deploy.id === undefined && (
          <Alert
            ...
            description={[
              <p key={1}>{t('Spug 将遵循先本地后目标主机的原则，按照顺序依次执行添加的动作，例如：本地动作1 -> 本地动作2 -> 目标主机动作1 -> 目标主机动作2 ...')}</p>,
              <p key={2}>{t('执行的命令内可以使用发布申请中设置的环境变量 SPUG_RELEASE，一般可用于标记一次发布的版本号或提交ID等，在执行的脚本内通过使用 $SPUG_RELEASE 获取其值来执行相应操作。')}</p>,
              <p key={3}>{Tips}{t('。')}</p>
            ]}/>
        )}
        {server_actions.map((item, index) => (
          <div key={index} style={{marginBottom: 30, position: 'relative'}}>
            <Form.Item required label={t('本地动作{}', index + 1)}>
              <Input disabled={store.isReadOnly} value={item['title']} onChange={e => item['title'] = e.target.value}
                     placeholder={t('请输入')}/>
            </Form.Item>

            <Form.Item required label={t('执行内容')}>
              <ACEditor
        ...
        {!store.isReadOnly && (
          <Form.Item wrapperCol={{span: 14, offset: 6}}>
            <Button type="dashed" block onClick={() => server_actions.push({})}>
              <PlusOutlined/>{t('添加本地执行动作（在服务端本地执行）')}
            </Button>
          </Form.Item>
        )}
        <Divider/>
        {host_actions.map((item, index) => (
          <div key={index} style={{marginBottom: 30, position: 'relative'}}>
            <Form.Item required label={t('目标主机动作{}', index + 1)}>
              <Input disabled={store.isReadOnly} value={item['title']} onChange={e => item['title'] = e.target.value}
                     placeholder={t('请输入')}/>
            </Form.Item>
            {item['type'] === 'transfer' ? ([
              <Form.Item key={0} required label={t('数据来源')}>
                <Input
                  spellCheck={false}
                  disabled={store.isReadOnly || item['src_mode'] === '1'}
                  placeholder={t('请输入本地（部署spug的容器或主机）路径')}
                  ...
                  addonBefore={(
                    <Select disabled={store.isReadOnly} style={{width: 120}} value={item['src_mode'] || '0'}
                      ...
                      <Select.Option value="0">{t('本地路径')}</Select.Option>
                      <Select.Option value="1">{t('发布时上传')}</Select.Option>
                    </Select>
                  )}/>
              </Form.Item>,
              [undefined, '0'].includes(item['src_mode']) ? (
                <Form.Item key={1} label={t('过滤规则')} extra={this.helpMap[item['mode']]}>
                  <Input
                    spellCheck={false}
                    placeholder={item['mode'] === '0' ? 'N/A' : t('请输入逗号分割的过滤规则')}
                    value={item['rule']}
                    ...
                    addonBefore={(
                      <Select disabled={store.isReadOnly} style={{width: 120}} value={item['mode']}
                        ...
                        <Select.Option value="1">{t('包含')}</Select.Option>
                        <Select.Option value="2">{t('排除')}</Select.Option>
                      </Select>
                    )}/>
                </Form.Item>
              ) : null,
        ...
        {!store.isReadOnly && (
          <Form.Item wrapperCol={{span: 14, offset: 6}}>
            <Button disabled={store.isReadOnly} type="dashed" block onClick={() => host_actions.push({})}>
              <PlusOutlined/>{t('添加目标主机执行动作（在部署目标主机执行）')}
            </Button>
            <Button
              block
              type="dashed"
        ...
        <Form.Item wrapperCol={{span: 14, offset: 6}} style={{marginTop: 24}}>
          <Button
            ...
            onClick={this.handleSubmit}>{t('提交')}</Button>
