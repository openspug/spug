...
export default observer(function () {
  ...
  const FilterHead = (
    <div style={{width: 512, display: 'flex', justifyContent: 'space-between'}}>
      <span>
        {t('文件过滤规则')} &nbsp;
        <Tooltip title={t('请输入相对于项目根目录的文件路径，根据包含或排除规则进行打包。')}>
      ...
      <Radio.Group
        ...
        <Radio.Button value="contain">
          <Tooltip title={t('仅打包匹配到的文件或目录，如果内容为空则打包所有。')}>{t('包含')}</Tooltip>
        </Radio.Button>
        <Radio.Button value="exclude">
          <Tooltip title={t('打包时排除匹配到的文件或目录，如果内容为空则不排除任何文件。')}>{t('排除')}</Tooltip>
        </Radio.Button>
      </Radio.Group>
    </div>
  )
  ...
  return (
    <Form layout="vertical" style={{padding: '0 120px'}}>
      ...
      <Form.Item
        label={t('代码检出前执行')}
        tooltip={t('在运行 Spug 的服务器(或容器)上执行，当前目录为仓库源代码目录，可以执行任意自定义命令。')}
        extra={<span>{Tips}{t('，请避免在此修改已跟踪的文件，防止在检出代码时失败。')}</span>}>
        <ACEditor
          readOnly={store.isReadOnly}
          mode="sh"
          theme="tomorrow"
      ...
      <Form.Item
        ...
        tooltip={t('在运行 Spug 的服务器(或容器)上执行，当前目录为检出后的源代码目录，可执行任意自定义命令。')}
        extra={<span>{Tips}{t('，大多数情况下在此进行构建操作。')}</span>}>
        <ACEditor
          readOnly={store.isReadOnly}
          mode="sh"
          theme="tomorrow"
      ...
      <Form.Item wrapperCol={{span: 14, offset: 6}}>
        <Button type="primary" onClick={handleNext}>{t('下一步')}</Button>
