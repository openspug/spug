...
export default observer(function () {
  ...
  return (
    <React.Fragment>
      <div className={styles.title}>{t('开放服务设置')}</div>
      <Form layout="vertical" style={{maxWidth: 320}}>
        <Form.Item colon={false} label={t('访问凭据')} extra={t('该自定义凭据用于访问平台的开放服务，例如：配置中心的配置获取API等，其他开放服务请查询官方文档。')}>
          <Input
            value={store.settings.api_key}
            onChange={e => store.settings.api_key = e.target.value}
            placeholder={t('请输入自定义凭证')}/>
        </Form.Item>
