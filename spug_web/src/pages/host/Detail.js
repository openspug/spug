  ...
  function handleTagConfirm(key) {
    if (tag) {
      if (key === 'disk') {
        ...
        if (lds.isNaN(value)) return message.error(t('请输入数字'));
  ...
  return (
    <Drawer
      ...
      <Descriptions
        ...
        title={<span style={{fontWeight: 500}}>{t('基本信息')}</span>}
        column={1}>
        <Descriptions.Item label={t('主机名称')}>{host.name}</Descriptions.Item>
        <Descriptions.Item label={t('连接地址')}>{host.username}@{host.hostname}</Descriptions.Item>
        <Descriptions.Item label={t('连接端口')}>{host.port}</Descriptions.Item>
        <Descriptions.Item label={t('独立密钥')}>{host.pkey ? t('是') : t('否')}</Descriptions.Item>
        <Descriptions.Item label={t('描述信息')}>{host.desc}</Descriptions.Item>
        <Descriptions.Item label={t('所属分组')}>
          <List>
      ...
      <Descriptions
        ...
        style={{marginTop: 24}}
        extra={edit ? ([
          <Button key="1" type="link" loading={fetching} icon={<SyncOutlined/>} onClick={handleFetch}>{t('同步')}</Button>,
          <Button key="2" type="link" loading={loading} icon={<SaveOutlined/>} onClick={handleSubmit}>{t('保存')}</Button>
        ]) : (
          <AuthButton auth="host.host.edit" type="link" icon={<EditOutlined/>} onClick={() => setEdit(true)}>{t('编辑')}</AuthButton>
        )}
        title={<span style={{fontWeight: 500}}>{t('扩展信息')}</span>}>
        <Descriptions.Item label={t('实例ID')}>
          {edit ? (
            <Input value={host.instance_id} onChange={e => handleChange(e, 'instance_id')} placeholder={t('选填')}/>
          ) : host.instance_id}
        </Descriptions.Item>
        <Descriptions.Item label={t('操作系统')}>
          {edit ? (
            <Input value={host.os_name} onChange={e => handleChange(e, 'os_name')}
                   placeholder={t('例如：Ubuntu Server 16.04.1 LTS')}/>
          ) : host.os_name}
        </Descriptions.Item>
        <Descriptions.Item label="CPU">
          {edit ? (
            <Input suffix={t('核')} style={{width: 100}} value={host.cpu} onChange={e => handleChange(e, 'cpu')}
                   placeholder={t('数字')}/>
          ) : host.cpu ? t('{}核', host.cpu) : null}
        </Descriptions.Item>
        <Descriptions.Item label={t('内存')}>
          {edit ? (
            <Input suffix="GB" style={{width: 100}} value={host.memory} onChange={e => handleChange(e, 'memory')}
                   placeholder={t('数字')}/>
          ) : host.memory ? `${host.memory}GB` : null}
        </Descriptions.Item>
        <Descriptions.Item label={t('磁盘')}>
          ...
          {edit && (inputVisible === 'disk' ? (
            ...
            <Tag className={styles.tagAdd} onClick={() => setInputVisible('disk')}><PlusOutlined/> {t('新建')}</Tag>
          ))}
        </Descriptions.Item>
        <Descriptions.Item label={t('内网IP')}>
          ...
          {edit && (inputVisible === 'sip' ? (
            ...
            <Tag className={styles.tagAdd} onClick={() => setInputVisible('sip')}><PlusOutlined/> {t('新建')}</Tag>
          ))}
        </Descriptions.Item>
        <Descriptions.Item label={t('公网IP')}>
          ...
          {edit && (inputVisible === 'gip' ? (
            ...
            <Tag className={styles.tagAdd} onClick={() => setInputVisible('gip')}><PlusOutlined/> {t('新建')}</Tag>
          ))}
        </Descriptions.Item>
        <Descriptions.Item label={t('实例计费方式')}>
          {edit ? (
            <Select
              style={{width: 150}}
              ...
              <Select.Option value="PrePaid">{t('包年包月')}</Select.Option>
              <Select.Option value="PostPaid">{t('按量计费')}</Select.Option>
              <Select.Option value="Other">{t('其他')}</Select.Option>
            </Select>
          ) : host.instance_charge_type_alias}
        </Descriptions.Item>
        <Descriptions.Item label={t('网络计费方式')}>
          {edit ? (
            <Select
              style={{width: 150}}
              ...
              <Select.Option value="PayByBandwidth">{t('按带宽计费')}</Select.Option>
        ...
        <Descriptions.Item label={t('到期时间')}>
          {edit ? (
            <DatePicker
        ...
        <Descriptions.Item label={t('更新时间')}>{host.updated_at}</Descriptions.Item>
      </Descriptions>
    </Drawer>
  )
})