...
import { includes, http, t } from 'libs';
        ...
        <div className={styles.right}>
          <Spin spinning={fetching}>
            ...
            <div style={{height: 540, overflow: 'auto'}}>
              ...
              {records.length === 0 &&
              <div className={styles.tips}>{t('该环境下还没有可发布或构建的应用哦，快去')}<Link to="/deploy/app">{t('应用管理')}</Link>{t('创建应用发布配置吧。')}</div>}
            </div>
          </Spin>
        </div>
      </div>
